...
def first_run(self):
    data = ('Emily','Parker','Emily Parker','503-281-5347','epark@gmail.com', 'Psychology 101')
    conn = sqlite3.connect('db_student_tracking.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_student_tracking (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('Emily','Parker','Emily Parker','503-281-5347','epark@gmail.com', 'Psychology 101'))
            conn.commit()
    conn.close()
